...
import { Todo, CreateTodoInput, UpdateTodoInput, Folder, CreateFolderInput, UpdateFolderInput } from '@/types'
...
export const folderAPI = {
  async getFolders(userId: string): Promise<Folder[]> {
    ...
    try {
      ...
      if (error) throw error
      // merge server folders with any local folders (local first)
      if (typeof window !== 'undefined') {
        const raw = window.localStorage.getItem(`local_folders_${userId}`)
        const local: Folder[] = raw ? JSON.parse(raw) : []
        return [...local, ...(data || [])]
      }
      return data || []
    } catch (err) {
  ...
  async createFolder(userId: string, input: CreateFolderInput): Promise<Folder> {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        const folder: Folder = {
          id: `local-folder-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
          name: input.name,
          color: input.color,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        ...
        const key = `local_folders_${userId}`
        const raw = window.localStorage.getItem(key)
        const arr: Folder[] = raw ? JSON.parse(raw) : []
        arr.push(folder)
        window.localStorage.setItem(key, JSON.stringify(arr))
        return folder
      }
  ...
  async updateFolder(folderId: string, input: UpdateFolderInput): Promise<Folder> {
    ...
    try {
      ...
      // local fallback
      if (typeof window !== 'undefined') {
        const keys = Object.keys(window.localStorage).filter((k) => k.startsWith('local_folders_'))
        for (const key of keys) {
          const raw = window.localStorage.getItem(key)
          if (!raw) continue
          const arr: Folder[] = JSON.parse(raw)
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        for (const key of keys) {
          ...
          if (!raw) continue
          const arr: Folder[] = JSON.parse(raw)
          const filtered = arr.filter((f) => f.id !== folderId)
          if (filtered.length !== arr.length) {
            window.localStorage.setItem(key, JSON.stringify(filtered))
            return
          }
