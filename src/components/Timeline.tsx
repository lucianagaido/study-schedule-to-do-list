...
export const Timeline: React.FC<TimelineProps> = ({ todos }) => {
  ...
  const tasksByDate = todosWithDates.reduce(
    (acc, todo) => {
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-500">
                  ({dayTasks.filter((t) => !t.completed).length} pending, {dayTasks.filter((t) => t.completed).length} done)
                </span>
              </div>
